...
import io.appwin.community.domain.CommunityPagination
...
import io.appwin.community.domain.CommunityUploadedMedia
...
public interface CommunityRepository {
  ...
  public suspend fun feed(
    ...
    cursor: String? = null,
    limit: Int = CommunityPagination.FEED_PAGE_SIZE,
  ...
  public suspend fun voteOnPoll(postId: String, optionId: String): CommunityPoll

  public suspend fun updatePost(
    postId: String,
    body: String? = null,
    groupId: String? = null,
    media: List<CommunityMedia>? = null,
  ): CommunityPost
  public suspend fun deletePost(postId: String)

  public suspend fun comments(
    postId: String,
    limit: Int = CommunityPagination.COMMENTS_PAGE_SIZE,
    offset: Int = 0,
  ...
  /** Sign → S3 → confirm; returns a public URL for posts / avatars. */
...
public class ApiCommunityRepository(
  ...
  override suspend fun feed(
    ...
    val query = buildList {
      ...
      authorProfileId?.let { add("authorProfileId=${it.encoded()}") }
  ...
  override suspend fun createPost(
    ...
    body = json.encodeToString(
      ...
      CreatePostBody(
        groupId = groupId,
        body = body,
        media = media.map { it.toMediaInput() },
        poll = pollOptions?.takeIf { it.size >= 2 }?.let { CreatePollBody(it) },
      ),
    ),
  ).toDomain()

  override suspend fun voteOnPoll(postId: String, optionId: String): CommunityPoll = client.request(
    path = "$BASE/posts/${postId.encoded()}/poll/votes",
    method = HttpMethod.POST,
    deserializer = PollVoteResultDto.serializer(),
    body = json.encodeToString(PollVoteBody.serializer(), PollVoteBody(optionId)),
  ).poll.toDomain()
  ...
  override suspend fun uploadMedia(
    ...
    val signed = client.request(
      ...
      method = HttpMethod.POST,
      deserializer = SignCommunityUploadResponse.serializer(),
      body = json.encodeToString(
        SignCommunityUploadBody.serializer(),
        SignCommunityUploadBody(mimeType, data.size),
      ),
    )
    AppwinCore.uploadPresigned(
      data = data,
      mimeType = mimeType,
      filename = filename,
    ...
    val confirmed = client.request(
      path = "$BASE/uploads/${signed.uploadId}/confirm",
      method = HttpMethod.POST,
      deserializer = ConfirmCommunityUploadResponse.serializer(),
    )
...
private fun CommunityMedia.toMediaInput(): MediaInputDto = MediaInputDto(
  url = url,
  width = width,
  height = height,
  alt = alt,
