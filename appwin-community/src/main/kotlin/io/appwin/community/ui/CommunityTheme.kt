 ...
 *
 * Accent colour, font, text scale and corner radius come from the dashboard.
 * Colour scheme is light-only (same as iOS / Support): a host app in dark mode
 * must not flip the feed to dark surfaces.
 *
 * System / host `fontScale` is locked to 1 here (same idea as iOS Dynamic Type
 * ignore). Otherwise a React Native or Samsung per-app text size inflates the
 * header and pills while cards look fine, and the feed no longer matches the
 * native host. Studio `theme.fontScale` still applies via [scaledTypography].
 */
@Composable
internal fun CommunityTheme(
  ...
  CompositionLocalProvider(
    LocalCommunityConfig provides config,
...
private val DefaultPrimary = CommunityColors.brand
