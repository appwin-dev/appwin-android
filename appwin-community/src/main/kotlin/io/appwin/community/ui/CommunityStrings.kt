...
internal class CommunityStrings(private val context: Context) {
  ...
  /**
   * Counted labels. `%d` is kept in the fallback so a studio overriding the key
   * keeps the placeholder; a count of zero falls back to the bare verb, which
   * is what the button means before anyone has acted on the post.
   */
  fun likeCount(count: Int): String =
    if (count <= 0) like else String.format(t("appwin_community_like_count", "%d Likes"), count)

  fun commentCount(count: Int): String =
    if (count <= 0) comment
    else String.format(t("appwin_community_comment_count", "%d Comments"), count)
