...
class AppwinNotificationsTest {
  ...
  @Test
  fun `onNewToken posts the token with the platform`() = runTest {
    server.enqueue(MockResponse().setResponseCode(200).setBody("""{"ok":true}"""))

    AppwinNotifications.onNewToken("fcm-token", pushOptIn = false)
  ...
  @Test
  fun `onNewToken rejects a blank token`() = runTest {
    val error = runCatching { AppwinNotifications.onNewToken("  ") }.exceptionOrNull()
  ...
  fun `un refus serveur remonte le statut`() = runTest {
    ...
    val error = runCatching {
      AppwinNotifications.onNewToken("fcm-token")
    }.exceptionOrNull()
