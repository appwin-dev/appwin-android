...
@Serializable
internal data class CachedVerdict(val verdict: AvailabilityVerdict, val fetchedAtMs: Long)
...
public class AvailabilityStore(context: Context, appId: String) {
  ...
  /**
   * Debug builds of the HOST app always revalidate, so the integration loop
   * (dashboard toggle, relaunch, ready) stays instant. The library's own
   * BuildConfig would not do: consumed as a release AAR from Maven, it says
   * "release" whatever the app builds.
   */
  private val alwaysRevalidate =
    (context.applicationInfo.flags and android.content.pm.ApplicationInfo.FLAG_DEBUGGABLE) != 0

  private var cached: CachedVerdict? = null

  private fun hydrate(): CachedVerdict? {
    cached?.let { return it }
    val raw = prefs.getString(key, null) ?: return null
    // A cache written by an older SDK (bare verdict, no timestamp) fails to
    // decode and counts as absent - one extra fetch, then the new format
    // takes over.
    return runCatching { json.decodeFromString(CachedVerdict.serializer(), raw) }
      .getOrNull()
      ?.also { cached = it }
  }

  private fun persist(verdict: AvailabilityVerdict) {
    val entry = CachedVerdict(verdict, System.currentTimeMillis())
    cached = entry
    runCatching {
      prefs.edit().putString(key, json.encodeToString(CachedVerdict.serializer(), entry)).apply()
    }
  }
   ...
   *
   * In release builds a verdict younger than [REVALIDATION_TTL_MS] spares the
   * network entirely: every end-user launch of every client app would
   * otherwise hit the endpoint for an answer that almost never changes. The
   * trade-off is explicit - a dashboard toggle flip reaches a device at the
   * first launch AFTER the TTL lapses, not the very next one.
   */
  private suspend fun fetch(client: ApiClient): AvailabilityVerdict? =
    mutex.withLock {
      if (!alwaysRevalidate) {
        hydrate()
          ?.takeIf { System.currentTimeMillis() - it.fetchedAtMs < REVALIDATION_TTL_MS }
          ?.let { return@withLock it.verdict }
      }
      val fresh =
        runCatching {
            client.request(
  ...
  private companion object {
    const val REVALIDATION_TTL_MS = 3_600_000L
  }
