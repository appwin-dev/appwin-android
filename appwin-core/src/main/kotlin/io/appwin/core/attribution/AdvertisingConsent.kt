 ...
 * relays an explicit GRANTED from its consent flow. Note the split with
 * ATT on iOS: this consent decides IF signals flow to networks at all,
 * ATT only decides whether the IDFA enriches them (refused ATT still
 * sends, at a lower match quality).
 */
public enum class AdvertisingConsent {
  GRANTED,
  DENIED,
  UNKNOWN,
}
