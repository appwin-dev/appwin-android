...
public class RealtimeHub private constructor(
  ...
  /**
   * Reopens a running connection with a freshly minted token. The token's
   * topics are those of the customer it was minted for, so after an identity
   * change the old socket would keep delivering the previous user's events.
   */
  ...
  private val socketListener = object : WebSocketListener() {
    ...
    override fun onClosed(webSocket: WebSocket, code: Int, reason: String) {
      // A socket replaced by `reconnect` must not schedule a second connection.
      if (webSocket !== this@RealtimeHub.webSocket) return
