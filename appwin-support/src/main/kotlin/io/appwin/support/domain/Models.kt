...
public data class Conversation(
  ...
  /**
   * Who wrote last, `null` on an empty conversation.
   *
   ...
   */
  public val lastMessageAuthorType: MessageAuthorType? = null,
  /** Last read by the customer, which drives the unread badge. */
  public val lastReadAtMillis: Long?,
  public val createdAtMillis: Long,
) {
  /**
   * True when support (studio / AI) wrote after the customer last read.
   *
   * The customer's own last message must not light the badge - only inbound
   * replies they have not opened yet.
   */
  public val hasUnread: Boolean
    get() {
      if (lastMessageAuthorType?.isStudio != true) return false
...
/**
 * Outbound attachment reference after Core upload (ADR-0022).
 *
 * Handed to `sendMessage` / `createConversation` - not a downloadable URL.
 */
public data class AttachmentInput(
...
public data class Customer(
  ...
  /** ISO 639-1 from identify / content detection. Drives date stamps in the UI. */
