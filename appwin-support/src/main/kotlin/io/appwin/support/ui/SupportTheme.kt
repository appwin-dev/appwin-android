 ...
 *
 * `agentTitle` needs [SupportStrings], which needs a context; the banner has
 * one, so it is built here rather than duplicating the fallback chain.
 */
internal fun MessengerConfig.agentDisplayName(context: Context): String {
  val strings = SupportStrings(context)
  return messaging.agentName ?: this.context.agentName.ifBlank { headerTitle(strings) }
}
