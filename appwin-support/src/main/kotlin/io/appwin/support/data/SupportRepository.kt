...
import io.appwin.support.SupportInAppWatcher
...
public interface SupportRepository {
  ...
  public suspend fun deleteMessage(conversationId: String, messageId: String)
  ...
  /** Customer typing → agents over HTTP (ADR-0028: socket is push-only). */
  public suspend fun emitTyping(conversationId: String, isTyping: Boolean)

  /** Fresh signed GET URL for an attachment (re-signed on tap). */
  public suspend fun freshAttachmentUrl(attachmentId: String): String

  public suspend fun faqGroups(): List<FaqGroup>

  /** The customer behind the current session, created as a lead if needed. */
  public suspend fun fetchCustomer(): Customer
}
...
public class ApiSupportRepository(
  ...
  override suspend fun conversations(cursor: String?, limit: Int): SupportPage<Conversation> {
    ...
    if (page.data.isNotEmpty()) SupportInAppWatcher.noteConversationsExist()
  ...
  override suspend fun createConversation(
    ...
    val conversation = client.request(
      path = "$BASE/conversations",
      method = HttpMethod.POST,
      deserializer = ConversationDto.serializer(),
      body = json.encodeToString(
        CreateConversationBody.serializer(),
        CreateConversationBody(SendMessageBody(firstMessage, attachments.toDto())),
      ),
    ).toDomain()
    SupportInAppWatcher.noteConversationsExist()
    return conversation
  }
  ...
  override suspend fun sendMessage(
    ...
    body = json.encodeToString(
      ...
      SendMessageBody(body, attachments.toDto()),
    ),
  ).toDomain()
  ...
  override suspend fun updateMessage(
    ...
    path = "$BASE/conversations/${conversationId.encoded()}/messages/${messageId.encoded()}",
    method = HttpMethod.PATCH,
    deserializer = MessageDto.serializer(),
    body = json.encodeToString(UpdateMessageBody.serializer(), UpdateMessageBody(body)),
  ).toDomain()
  ...
  override suspend fun emitTyping(conversationId: String, isTyping: Boolean) {
    client.requestVoid(
      path = "$BASE/conversations/${conversationId.encoded()}/typing",
      method = HttpMethod.POST,
      body = json.encodeToString(TypingBody.serializer(), TypingBody(isTyping)),
    )
  }

  override suspend fun freshAttachmentUrl(attachmentId: String): String = client.request(
    path = "$BASE/attachments/${attachmentId.encoded()}/url",
    method = HttpMethod.GET,
    deserializer = AttachmentUrlDto.serializer(),
  ).url
  ...
  // Attributes now go through Core's `PATCH /v1/me`; an empty identify body
  // only reads the customer back.
  override suspend fun fetchCustomer(): Customer = client.request(
    path = "$BASE/identify",
    method = HttpMethod.POST,
...
private fun List<AttachmentInput>.toDto(): List<AttachmentInputDto> = map {
  AttachmentInputDto(
