...
internal data class ConversationDto(
  ...
  val lastMessageAuthorType: String? = null,
  ...
  fun toDomain(): Conversation = Conversation(
    ...
    lastMessageAuthorType = lastMessageAuthorType?.let { MessageAuthorType.from(it) },
...
internal data class CustomerDto(
  ...
  fun toDomain(): Customer = Customer(id, externalId, name, email, avatarUrl, language)
}
...
internal data class MessengerConfigDto(
  ...
  fun toDomain(): MessengerConfig = MessengerConfig(
    ...
    design = MessengerDesign(
      ...
      bannerSource = MessengerBannerSource.from(design?.bannerSource),
      presetBanner = MessengerPresetBanner.from(design?.presetBannerId),
      ...
      bannerFocusY = design?.bannerFocusY?.toFloat() ?: 50f,
    ),
...
@Serializable
internal data class AttachmentInputDto(
...
@Serializable
internal data class UpdateMessageBody(val body: String)

@Serializable
internal data class ToggleReactionBody(val emoji: String)

@Serializable
internal data class TypingBody(val isTyping: Boolean)

@Serializable
internal data class AttachmentUrlDto(val url: String)
