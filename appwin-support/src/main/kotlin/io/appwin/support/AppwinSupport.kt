...
public object AppwinSupport {
   ...
   */
  @JvmStatic
  public suspend fun loginUnidentifiedUser(): Customer {
    AppwinCore.bootstrapSession()
    val next = repository.identify(SupportUserAttributes())
    customer = next
    return next
  }
   ...
   * alone. Intercom model: opening the messenger creates a lead.
   */
  @JvmStatic
  public suspend fun ensureCustomer(): Customer {
   ...
   *
   * The identity is owned by Core and shared by every product; Support also
   * promotes the lead server-side via `/identify`, same sequence as on iOS.
   */
  @JvmStatic
  public suspend fun loginIdentifiedUser(externalId: String): Customer {
    require(externalId.isNotEmpty()) { "externalId is empty" }
  ...
  public suspend fun updateUser(attributes: SupportUserAttributes): Customer {
    val next = repository.identify(attributes)
    customer = next
    return next
  }
  ...
  @JvmStatic
  public suspend fun refreshCustomer(): Customer {
    val next = repository.identify(SupportUserAttributes())
    customer = next
    return next
  }
