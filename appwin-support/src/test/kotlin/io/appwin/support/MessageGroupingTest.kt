...
class MessageGroupingTest {
  ...
  @Test
  fun `trois messages du meme auteur d affilee ne montrent qu un horodatage`() {
  ...
  @Test
  fun `plusieurs messages d affilee restent groupes meme entre minutes`() {
    // Dashboard / iOS: same author consecutive = one run, even across minutes.
    val out = groupMessages(
      listOf(
        message("a", MessageAuthorType.CUSTOMER, minute),
        message("b", MessageAuthorType.CUSTOMER, 2 * minute),
        message("c", MessageAuthorType.CUSTOMER, 3 * minute),
      ),
    )
